...
<?php
    ...
    {
        $sql3 = "SELECT *  FROM `movies` WHERE `genre` LIKE '$rowk[gnm]' OR `genret` LIKE '$rowk[gnm]';";
        $cnt=dbset($sql3,2);
        if($cnt!=0)
        {
...
if($kid==1)
{
  // $sql = "SELECT *  FROM `movies` WHERE `feature` LIKE 'On' AND `kids` LIKE 'On';";
   $sql2 = "SELECT *  FROM `movies` WHERE `genre` LIKE '$rowk[gnm]' OR `genret` LIKE '$rowk[gnm]' AND `kids` LIKE 'On';";
}
else
{
   $sql2 = "SELECT *  FROM `movies` WHERE `genre` LIKE '$rowk[gnm]' OR `genret` LIKE '$rowk[gnm]';";
