<?php
    ...
    {
        $mid=$_POST['txtmid'];
        $qry="insert into latest(mid) values('$mid')";
        mysqli_query($conn,$qry);
        echo "<script>alert('Movie Added to latest.');</script>";
    }

    if(isset($_POST['remove']))
    {
        $mid=$_POST['txtmid'];
        $qry="delete from latest where mid='$mid'";
        mysqli_query($conn,$qry);
        echo "<script>alert('Movie removed to latest.');</script>";
    }
?>
<script>
function myFunction() {
                                            ...
                                            <table class="table mb-0" id="dtBasicExample">
                                                ...
                                                <?php
                                                    ...
                                                    while($row=mysqli_fetch_array($rs))
                                                    {
                                                        $r+=1;
                                                        $mid=$row['mid'];
                                                        $ccq="select mid from latest where mid='$mid'";
                                                        ...
                                                        <td>
                                                            <form action="#" method="post">
                                                                ...
                                                                <?php
                                                                    ...
                                                                    {
                                                                ?>
                                                                <button type="submit" name="set" class="btn btn-primary waves-effect waves-light">Set as Latest</button>
                                                                <?php
                                                                    }
                                                                    else
                                                                    {
                                                                ?>
                                                                <button type="submit" name="remove" class="btn btn-primary waves-effect waves-light">Remove from Latest</button>
                                                                <?php
                                                                    }
                                                                ?>
                                                            </form>
